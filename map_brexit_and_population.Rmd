...
gg <- ggplot() + geom_polygon(data = df, aes(x = long, y = lat, group = group, fill = decision_for_Brexit), color = "#FFFFFF", size = 0.25)
gg <- gg + coord_fixed(1)
gg <- gg + theme_minimal()
...
gg <- ggplot() + geom_polygon(data = df, aes(x = long, y = lat, group = group, fill = odds_Brexit), color = "#FFFFFF", size = 0.25)
