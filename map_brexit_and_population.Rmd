...
gg <- ggplot() + geom_polygon(data = df, aes(x = long, y = lat, group = group, fill = decision_for_Brexitregional), color = "#FFFFFF", size = 0.25)
gg <- gg + coord_fixed(1)
gg <- gg + theme_minimal()
...
gg <- ggplot() + geom_polygon(data = df, aes(x = long, y = lat, group = group, fill = odds_region_brexit), color = "#FFFFFF", size = 0.25)
gg <- gg + scale_fill_gradientn(colours = c("green", "blue", "red"))
gg <- gg + coord_fixed(1)
gg <- gg + theme_minimal()
